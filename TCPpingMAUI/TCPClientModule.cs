...
namespace TCPpingMAUI
{
    public interface ITCPClientModule
    {
        ...
        /// <summary>
        /// Get the server IP address that responded to the ping.
        /// </summary>
        /// <returns></returns>
    ...
    public class TCPClientModule : ITCPClientModule
    {
        ...
        { 
            ...
            set 
            {
                ...
                if (value != null)
                {
                    Console.WriteLine($"[TCP Client] Đã tìm thấy máy chủ: {value}");
                }
            } 
        }
        public TCPClientModule(int port)
        {
            ...
            string[]? Octet = _ipControl.GetOctet();
        ...
        private void PingMessage(IPAddress ip,  byte[] pingMessage, out bool stop)
        {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    if (_ipControl.IsValidIPv4(response)) 
                    { 
                        flag = true; 
                        IPAddress.TryParse(response, out serverIP);
                        ServerIP = serverIP;
                    }
                }
                catch { }
